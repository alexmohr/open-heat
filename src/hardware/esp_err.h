...
#define ESP_ERR_WIFI_NOT_INIT                                                            \
  (ESP_ERR_WIFI_BASE + 1) /*!< WiFi driver was not installed by esp_wifi_init */
...
#define ESP_ERR_WIFI_CONN                                                                \
  (ESP_ERR_WIFI_BASE + 7) /*!< WiFi internal control block of station or soft-AP error   \
                           */
...
#define ESP_ERR_WIFI_WAKE_FAIL                                                           \
  (ESP_ERR_WIFI_BASE + 13) /*!< WiFi is in sleep state(RF closed) and wakeup fail */
...
#define ESP_ERR_WIFI_PM_MODE_OPEN                                                        \
  (ESP_ERR_WIFI_BASE + 18) /*!< Wifi is in min/max modem sleep mode */
 ...
 * @param buflen Size of buffer buf. At most buflen bytes are written into the buf buffer
...
/* This won't happen on IDF, which defines __ASSERT_FUNC in assert.h, but it does happen
   when building on the host which uses /usr/include/assert.h or equivalent.
*/
...
#define ESP_ERROR_CHECK(x)                                                               \
  do {                                                                                   \
    esp_err_t __err_rc = (x);                                                            \
    if (__err_rc != ESP_OK) {                                                            \
      _esp_error_check_failed(__err_rc, __ESP_FILE__, __LINE__, __ASSERT_FUNC, #x);      \
    }                                                                                    \
  } while (0);
#endif

/**
 * Macro which can be used to check the error code. Prints the error code, error location,
 * and the failed statement to serial output. In comparison with ESP_ERROR_CHECK(), this
 * prints the same error message but isn't terminating the program.
 */
#ifdef NDEBUG
  ...
  ({                                                                                     \
    ...
    if (__err_rc != ESP_OK) {                                                            \
      _esp_error_check_failed_without_abort(                                             \
        __err_rc, __ESP_FILE__, __LINE__, __ASSERT_FUNC, #x);                            \
    }                                                                                    \
    __err_rc;                                                                            \
  })
#endif // NDEBUG
